 ...
 *  
 */
public class PlayerArmBowAssets extends EntityAssets {
    ...
    public void init() {
        ...
        BufferedImage[] player_down_attack = new BufferedImage[13];
        BufferedImage[] player_up_attack = new BufferedImage[13];
        BufferedImage[] player_left_attack = new BufferedImage[13];
        BufferedImage[] player_right_attack = new BufferedImage[13];
        ...
        int startingYBowAttack = 16 * height;
        int startingYWalk = 8 * height;
        SpriteSheet sheet1, sheet2, sheet = null;
        sheet = new SpriteSheet(ImageLoader.loadImage("/images/assets/player_arm_bow.png"));

        for (int i = 0; i < 13; i++) {

            player_down_attack[i] = sheet.crop(startingXAttack + i * width, startingYBowAttack + 2 * height, width, height);
            player_right_attack[i] = sheet.crop(startingXAttack + i * width, startingYBowAttack + 3 * height, width, height);
            player_up_attack[i] = sheet.crop(startingXAttack + i * width, startingYBowAttack, width, height);
            player_left_attack[i] = sheet.crop(startingXAttack + i * width, startingYBowAttack + height, width, height);
